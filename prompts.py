...
7. 如果涉及多表关联：
   - 必须给每张表起别名
   - 所有列名必须加 `表别名.列名` 前缀，绝对禁止出现 ambiguous column 错误
   - 尽量用 LEFT JOIN / INNER JOIN，不要隐式逗号联接
8. 所有表名、字段名用反引号包裹
...
12. **字段取值范围（非常重要）**：
    - `gross_margin`, `net_margin`, `roe`, `roa`, `debt_ratio`, `avg_roe`, `avg_gross_margin` 等**比率类字段**，存的是 `0~1 小数`（如 85% = 0.85），**不要用 85 去比较**
    - `market_cap`（市值）存的是**亿元**（如 21000.0 = 2.1 万亿元），不要按"元"去除以 1 万亿
    - `revenue`, `net_profit` 等金额字段存的也是**亿元**
13. 当问题问"谁最高/最低/排第几/对比/排名"时，返回完整的排序结果（3 家公司都返回）用 `LIMIT 3`，不要只返回 1 行，避免丢失上下文
